		...
		$.ajax({
			...
			success: function(respuesta){
				...
				for(var i = 0; i < precioCarritoCompra.length; i++){
					...
					$(".subTotal"+i).html('<strong> $<span>'+(precioCarritoCompraArray*cantidadItemArray)+'</span></strong>')
...
/*
	...
	*/
	...
	if(agregarAlCarrito){
		...
		swal({
			  title: "",
			  text: "¡Se ha agregado un nuevo producto al carrito de compras! \nLa cantidad la modificas en tu carrito.",
			  type: "success",
...
function divisas(metodoPago){
	...
	if(metodoPago == "paypal"){
		...
		$("#cambiarDivisa").append('<option value=""></option>'+
...
$("#cambiarDivisa").change(function(){
	...
	$.ajax({
	    ...
	    success:function(respuesta){
	    	...
	    	var conversion = (respuesta["_"+divisa]["val"]).toFixed(2);
